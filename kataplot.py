...
parser.add_option('--vmin', dest = 'vmin', help = 'Minimum value for colour scale (default = data minimum)', default = 0.0)
parser.add_option('--vmax', dest = 'vmax', help = 'Maximum value for colour scale (default = data maximum)', default = '')
...
for datfile in to_plot:
	...
	datamin = numpy.min(raw[~numpy.isnan(raw)])
	datamax = numpy.max(raw[~numpy.isnan(raw)])
	print('   Data min: '+str(datamin))
	print('   Data max: '+str(datamax))

	# Sort out vmax/vmin if not supplied by user
	if vmax != '':
		vmax = float(vmax)
	else:
		vmax = datamax
